...
{
	[ActionCategory("FOV")]
	[Tooltip("Visually create a FOV mesh and set bool if target is inside gameobject FOV.")]
	[HelpUrl("http://hutonggames.com/playmakerforum/index.php?topic=11753.0")]

	public class DynamicVisionConeFov : FsmStateAction 
	
	{
		...
		[Tooltip("Will set GameObject to found target")]
		public FsmGameObject foundTarget;

		public bool isOnPursuit;
		...
		void CastRays()
		{
			...
			{
				direction = Quaternion.AngleAxis(currentAngle, go.gameObject.transform.up) * go.gameObject.transform.forward;

				hit = new RaycastHit();

				if (useTag.Value == false){

				if(Physics.Raycast(go.gameObject.transform.position, direction, out hit, fovMaxDistance.Value, ActionHelpers.LayerArrayToLayerMask(layerMask, invertMask.Value)) == false)
				{
					...
					if(Physics.Raycast(go.gameObject.transform.position, direction, out hit, fovMaxDistance.Value, ActionHelpers.LayerArrayToLayerMask(layerMask, invertMask.Value)) == false)
					{
						...
						{
							if (hit.collider.CompareTag(filterTag.Value))
							{
								hit.point = go.gameObject.transform.position + (direction * fovMaxDistance.Value);
							}
						...
						{
							Debug.DrawRay(go.transform.position,direction*fovMaxDistance.Value,Color.white,0.1f);
		...
		void UpdateMesh()
		{
			...
			{
				newVertices[i] = go.gameObject.transform.InverseTransformPoint(hits[i-1].point);
			}
		...
		void Output()
		{
			...
			foreach (RaycastHit hit in hits)
			{
				if (hit.transform == target.Value.transform)
				{
					...
					foundTarget.Value = hit.collider.gameObject;
				}
			}
				...
				foreach (RaycastHit hit in hits)
				{
					...
					{
						Debug.DrawLine(go.transform.position,hit.point,Color.red,0.1f);
						insideFov.Value = true;
						foundTarget.Value = hit.collider.gameObject;
					}
					else
					{
						Debug.DrawLine(go.transform.position,hit.point,Color.white,0.1f);
					}
				}
