...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class GetMotionBlur : FsmStateAction
	{
        ...
        private void ggop()
        {
            ...
            else
            {
                ...
                if (!EnableValue.IsNone)
                    EnableValue.Value=motionBlur.enabled.value;
                if (!ShutterAngleValue.IsNone)
                    ShutterAngleValue.Value=motionBlur.shutterAngle.value;
                if (!SampleCountValue.IsNone)
                    SampleCountValue.Value=motionBlur.sampleCount.value;
