...
namespace HutongGames.PlayMaker.Actions
{
	...
	public class SetIntensityMaterialColor : ComponentAction<Renderer>
	{
		...
		[RequiredField]
		[Tooltip("Set emission intensity")]
		...
		void DoSetMaterialColor()
		{
			...
			float intensity = intensityValue.Value;
			var colorName = namedColor.Value;
			if (colorName == "") colorName = "_Color";

			if (material.Value != null)
			{
				material.Value.SetColor(colorName, color.Value * intensity);
				return;
			}
			...
			{
				renderer.material.SetColor(colorName, color.Value * intensity);
			}
			else if (renderer.materials.Length > materialIndex.Value)
			{
				var materials = renderer.materials;
				materials[materialIndex.Value].SetColor(colorName, color.Value * intensity);
